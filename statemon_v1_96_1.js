...
let statemon = {
	...
	notes: {
		...
		// v1.94: improved css
		// v1.95: made some changes
		// v1.96: lost the changes, did them again, tried to combine the two, went with just the new one
		// ----
	...
	addTheStyles()
	{
		...
		if(statemon.styles)
		{
			...
			statemon.styles.innerHTML = `
                ...
                .scb-changed:hover { background: white; }
                .scb-added:hover { background: white; }
	...
	monitor()
	{
		...
		if(statemon.state)
		{
			...
			statemon.monitorInterval = setInterval(() =>
			{
				...
				{
					clearInterval(statemon.monitorInterval);
					statemon.monitorInterval = null;
					console.log('Monitoring finished - toggled by user.', statemon.deathCount);
					statemon.deathCount += 1;
				}
				else if(statemon.shouldDieBecauseTooMuchRam())
				{
					clearInterval(statemon.monitorInterval);
					statemon.monitorInterval = null;
					console.log('Monitoring finished - too much RAM.', statemon.deathCount);
					statemon.deathCount += 1;
			        statemon.trimChanges();
				}

				if(statemon.monitorInterval && statemon.togglers.monitoring)
				{
					statemon.monitorFn();
				}

				if(statemon.deathCount > 5)
				{
					console.log('deaths:', statemon.deathCount, '!');
					statemon = {};
				}
	...
	shouldDieBecauseTooMuchRam()
	{
		...
		if(statemon.togglers.showAdded && statemon.currentRamUsage() > 250)
		{
			statemon.toggleHideAdded();
		}
